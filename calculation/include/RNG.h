...
public:
	...
	// unsigned integers
	// N shoulde be N > 0
	inline uint64_t lessThan(uint64_t N) // [0, N)
	{
	...
	// N should be N > 0
	template <uint64_t N>
	inline uint64_t lessThan() // [0, N), slightly faster with constexpr
	{
	...
	// integers
	// should be A <= B
	inline int64_t between(int64_t A, int64_t B) // [A, B]
	{
	...
	inline int64_t between() // [A, B], slightly faster with constexpr
	{
	...
	// should be A < B
	inline float64_t ieRange(float64_t A, float64_t B) // [A, B)
	{
	...
	// should be A < B
	inline float64_t eiRange(float64_t A, float64_t B) // (A, B]
	{
	...
	// should be A < B
	inline float64_t eeRange(float64_t A, float64_t B) // (A, B)
	{
	...
	// should be A <= B
	inline float64_t iiRange(float64_t A, float64_t B) // [A, B]
	{
